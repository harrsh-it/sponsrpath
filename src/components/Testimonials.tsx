...
export function Testimonials() {
  return (
    <section className="py-8 sm:py-10 lg:py-8 bg-[#f4f9fb]">
      <div className="w-full mx-auto px-4 sm:px-8 lg:px-16">
        ...
        >
          ...
          <p className="mt-3 sm:mt-4 max-w-xl mx-auto text-base sm:text-lg lg:text-xl font-semibold text-gray-600 px-2 sm:px-0">
            Join thousands of international students who found sponsored roles
            faster and smarter.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {MOCK_TESTIMONIALS.map((t, i) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.1 }}
              className="bg-white border rounded-2xl p-5 sm:p-6 flex flex-col gap-4 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all"
            >
              {/* Quote Icon */}
              <Quote className="w-8 h-8 sm:w-10 sm:h-10 text-[#1a9889]/60" />

              {/* Text */}
              <p className="text-base sm:text-lg text-center text-gray-600 leading-relaxed flex-1">
                "{t.quote}"
              </p>
              ...
              <div className="flex items-center gap-3 pt-3 border-t">
                <img
                  src={t.avatar}
                  alt={t.name}
                  className="w-14 h-14 sm:w-20 sm:h-20 rounded-full border flex-shrink-0"
                />

                <div>
                  <p className="text-base sm:text-lg font-semibold text-[#09567a]">
                    {t.name}
                  </p>
                  ...
                  <p className="text-sm sm:text-base font-medium text-[#147f8a] mt-0.5">
                    {t.role}
                  </p>
                </div>
              </div>

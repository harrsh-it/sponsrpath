...
export function Footer() {
  return (
    <footer className="bg-black text-white pt-10 sm:pt-14 lg:pt-16 pb-8">
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-16">

        {/* Top: Logo + Links */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 sm:gap-8 pb-10 sm:pb-12 border-b border-white/10">
            ...
            <div className="flex items-center gap-2 sm:gap-3 mt-4 sm:mt-6">
              {SOCIAL_LINKS.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label={label}
                  className="w-9 h-9 sm:w-12 sm:h-12 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors"
                >
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title}>
              <h3 className="text-sm sm:text-base lg:text-xl font-bold uppercase tracking-widest text-white/40 mb-3 sm:mb-4">
                {title}
              </h3>
        ...
        <div className="pt-6 sm:pt-8 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          <p className="text-sm sm:text-base lg:text-lg text-white/40 text-center sm:text-left">
            © {new Date().getFullYear()} SponsorPath Ltd. Registered in England &amp; Wales. All rights reserved.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-x-3 sm:gap-x-4 gap-y-1">
            {LEGAL_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-xs sm:text-sm lg:text-lg text-white/40 hover:text-white/70 transition-colors"
              >
                {link.label}
              </Link>
            ))}
        ...
        <p className="text-center text-sm sm:text-base lg:text-xl text-white/30 mt-4 sm:mt-6">
          🔒 GDPR compliant · ICO registered · We never sell your data · Right to be forgotten guaranteed
        </p>
      </div>
    </footer>

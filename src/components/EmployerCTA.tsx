...
export function EmployerCTA() {
  return (
    <section className="py-8 sm:py-10 lg:py-8 relative overflow-hidden">
      {/* subtle gradient overlay */}
      <div className="absolute inset-0 opacity-60" />

      <div className="relative w-full mx-auto px-4 sm:px-8 lg:px-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12 items-center">
          ...
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <p className="text-base sm:text-xl text-[#1a9889] font-semibold mb-2 sm:mb-4">
              For Employers
            </p>
            ...
            <p className="text-black/80 mb-6 sm:mb-8 max-w-xl leading-relaxed text-base sm:text-xl font-semibold">
              Stop wading through hundreds of CVs. Search verified talent from
              top universities — filtered by skill, availability, and graduation year.
            </p>
            ...
            <ul className="space-y-3 sm:space-y-4 mb-8 sm:mb-10">
              {VALUE_PROPS.map(({ icon: Icon, text }) => (
                <li
                  key={text}
                  className="flex items-center gap-3 text-black/80 text-base sm:text-xl font-semibold"
                >
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0">
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-[#1a9889]" />
                  </div>
                  {text}
                </li>
              ))}
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3">
              <Link href="/employers" className="w-full sm:w-auto">
                <Button className="w-full sm:w-auto bg-[#1a9889] hover:bg-[#147f8a] text-black text-base sm:text-xl font-semibold">
                  Get Started Free
                  <ArrowRight className="ml-2 w-4 h-4" />
                </Button>
              </Link>
              ...
              <Link href="/talent" className="w-full sm:w-auto">
                <Button className="w-full sm:w-auto border border-white/30 text-black text-base sm:text-xl font-semibold hover:bg-white/10">
                  Browse Talent
                </Button>
              </Link>
            </div>
          ...
          >
            {STATS.map((stat) => (
              <div
                key={stat.label}
                className="bg-gray-200 backdrop-blur-md border border-white/20 rounded-2xl p-4 sm:p-6 text-center hover:bg-white/20 transition"
              >
                <p className="text-2xl sm:text-3xl font-bold text-[#1a9889] mb-1 sm:mb-2">
                  {stat.value}
                </p>
                <p className="text-sm sm:text-lg font-semibold text-black/70">
                  {stat.label}
                </p>
              </div>
            ))}

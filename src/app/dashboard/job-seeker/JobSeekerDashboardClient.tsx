  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-10">
      ...
      >
        <div>
          ...
          <p className="text-slate-500 font-bold mt-1">
            You have <span className="text-teal font-black">{recommendedJobs.length} new matches</span> and <span className="text-teal font-black">{interviews.length} interview</span> today.
          </p>
        </div>
        <div className="flex items-center gap-6">
          <ProfileAvatar 
            ...
            className="w-16 h-16 shadow-xl shadow-navy/10"
          />
          ...
          <Link 
            ...
            className="hidden sm:flex items-center gap-3 bg-navy text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-teal transition-all shadow-xl shadow-navy/10 group h-fit"
          >
            <Zap className="h-4 w-4 fill-white group-hover:animate-pulse" /> Complete Profile
          </Link>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {[
          { label: "Total applied", value: applications.length.toString(), sub: "+0 this week", icon: Briefcase, color: "text-teal", bg: "bg-teal/5" },
          { label: "Profile views", value: "0", sub: "+0 this week", icon: Users, color: "text-blue-500", bg: "bg-blue-50/50" },
          { label: "Interviews", value: interviews.length.toString(), sub: `${interviews.length} upcoming`, icon: Calendar, color: "text-indigo-500", bg: "bg-indigo-50/50" },
          { label: "Profile strength", value: `${jobSeeker.profileStrength}%`, sub: "Add skills", icon: Award, color: "text-amber", bg: "bg-amber/5" }
        ].map((stat, i) => (
          <motion.div 
            key={i}
          ...
          >
            ...
            <div className="flex items-baseline gap-3 relative z-10">
              <span className="text-4xl font-heading font-black text-navy tabular-nums">{stat.value}</span>
              <span className="text-[10px] font-black text-teal bg-teal/10 px-2 py-1 rounded-lg uppercase tracking-wider">{stat.sub}</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-heading font-black text-navy flex items-center gap-3">
                <Search className="h-6 w-6 text-teal" /> Recommended for you
              </h2>
              <Link href="/jobs" className="text-xs font-black text-teal hover:text-navy uppercase tracking-widest flex items-center gap-1 transition-colors">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                      ...
                      <div className="flex gap-2">
                        <button className="p-2.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all cursor-pointer">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-heading font-black text-navy flex items-center gap-3">
                <Clock className="h-6 w-6 text-teal" /> Your Applications
              </h2>
              <Link href="#" className="text-xs font-black text-teal hover:text-navy uppercase tracking-widest flex items-center gap-1 transition-colors">
                View all <ExternalLink className="h-3 w-3" />
              </Link>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        ...
                        <tr key={app.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer group">
                            ...
                            <div className="flex justify-center">
                                ...
                                app.status === 'Offer' ? 'bg-emerald/10 text-emerald border-emerald/20' :
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <section>
              <h2 className="text-2xl font-heading font-black text-navy mb-6">Quick actions</h2>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-heading font-black text-navy">Match strength</h2>
                <Link href="/dashboard/job-seeker/profile/edit" className="text-[10px] font-black text-teal flex items-center gap-1 hover:underline uppercase tracking-widest">
                    Improve <Plus className="h-3 w-3" />
                </Link>
              </div>
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          {/* Profile Completion */}
          <section className="bg-linear-to-br from-navy to-teal p-8 rounded-[2.5rem] text-white shadow-2xl shadow-teal/20 relative overflow-hidden group">
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-black font-heading uppercase tracking-widest">Verification</h3>
                    <Link href="/dashboard/job-seeker/profile/edit" className="text-[10px] font-black bg-white/20 hover:bg-white text-white hover:text-navy px-4 py-1.5 rounded-xl transition-all uppercase tracking-widest shadow-xl backdrop-blur-md">
                        Boost ↗
                    </Link>
                </div>
                ...
                <div className="space-y-4">
                    {jobSeeker.checklist.map((item, i) => (
                        <div key={i} className="flex items-center gap-4">
                            ...
                            <span className={`text-xs font-black uppercase tracking-widest ${item.completed ? 'text-white' : 'text-white/40'}`}>{item.label}</span>
                        </div>
                    ))}
                </div>
            </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xl font-heading font-black text-navy uppercase tracking-widest">Interviews</h3>
              <Link href="#" className="text-[10px] font-black text-teal hover:text-navy uppercase tracking-widest transition-colors">View all</Link>
            </div>
            {interviews.length > 0 ? (
          ...
          <section>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xl font-heading font-black text-navy uppercase tracking-widest">Feed</h3>
              <button className="text-[10px] font-black text-slate-300 hover:text-navy uppercase tracking-widest transition-all cursor-pointer">Mark read</button>
            </div>
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-navy/5 overflow-hidden divide-y divide-slate-50">
              {notifications.length > 0 ? (
                notifications.map((notif) => (
                    <div key={notif.id} className={`p-6 hover:bg-slate-50 transition-all cursor-pointer relative group ${!notif.isRead ? 'bg-teal/5' : ''}`}>
                    {!notif.isRead && <div className="absolute left-0 top-0 bottom-0 w-1 bg-teal shadow-xl shadow-teal/50"></div>}
                    <p className="text-xs font-bold text-navy leading-relaxed mb-2 group-hover:text-teal transition-colors">{notif.text}</p>
                ...
                <div className="p-16 text-center">
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">No new alerts.</p>
                </div>
              )}
            </div>
          </section>
